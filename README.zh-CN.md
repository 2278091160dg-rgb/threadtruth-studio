...
- 单风格预览覆盖：**历史原生尝试3次；机器版式通过1/24，人工视觉验收1/24，公开0/24**。目标仍是**同一白马甲，24种风格各一张预览，每张一种风格、六个姿势**。开发层 schema `4.0` 保留原生整板，并制作明确标注的排版衍生图：整板 `1200×1200`、六个 `360×480`（`3:4`）卡片。完整姿势画面等比放入，不拉伸、不放大、不裁人物；允许留白和本地排字，但不宣称原生模型精确控制版式。旧 schema 1–3 记录只读，原生比例失败结论保留。已复用登记的韩系C原图完成排版及机器检查，不新增生图，维护者已确认视觉验收通过；正式公开审核与发布授权仍待确认，其余23种风格分别取得生成授权。完整进度见[任务台账](docs/WORK-STATUS.md)。
