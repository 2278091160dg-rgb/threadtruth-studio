...
- 单风格预览覆盖：**原生尝试 1 次，合格 0/24，人工验收 0/24，公开 0/24**。目标是**同一件白色马甲，24 个风格各一张预览；每张只有一种风格、六个姿势**。整张原生看板（标题、副标题、六格图片区和页脚都在内）必须是 `1:1` 正方形；`3:4` 指每个姿势小格，不是整块图片区。当前 schema `3.0` 收集器按风格绑定源图／规则／风格包、动作 0 精确提示词、原生尺寸、不可变版式与标签哈希、实测几何和单独验收记录；工具本身不生图、不代填验收。schema `1.0`、`2.0` 运行仅供历史只读查看，不能修改或晋级。先前错误的混风格看板与这 1 次不合格单风格尝试分开记账。下一门禁是另行明确授权的韩系冷感 C 模式纠正重试；实际通过后才能继续其余 23 风格。若都不失败，仍需 24 次新的初始调用；已消耗尝试另记，失败重试需额外授权。软件或文档完成不增加成图计数。
