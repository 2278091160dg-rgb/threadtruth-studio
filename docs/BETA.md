...
The corrected preview collection is 24 separate single-style, six-pose contact sheets of the same white vest, using the runtime's existing action 0. On each native square board, the title, subtitle, six-cell picture area and footer are all inside the `1:1` boundary; each pose cell is `3:4`. The schema `3.0` developer collector and whole-sheet publication validator are locally implemented, but local software closure is not image qualification. Current image accounting is one unqualified single-style native attempt, `0/24` qualified, `0/24` human-approved and `0/24` public; the earlier wrong mixed-style board is separate. Schema `1.0` and `2.0` records are superseded, inspection-only history and cannot be promoted. Preview acceptance cannot satisfy the three-complete-workflow target. See the [total task register and schedule](WORK-STATUS.md).
