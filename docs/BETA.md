...
The corrected preview collection is 24 separate single-style, six-pose contact sheets of the same white vest, using the runtime's existing action 0. Approved preview coverage is `0/24`; independent six-final case coverage is `1/24`. The beta.2 four-mixed-style-board developer workflow is superseded and awaits replacement. Preview acceptance cannot satisfy the three-complete-workflow target. See the [total task register and schedule](WORK-STATUS.md).
