...
The corrected preview collection is 24 separate single-style, six-pose contact sheets of the same white vest, using the runtime's existing action 0. The v2 developer collector and whole-sheet publication validator are available, but they contain no generated or human-approved preview evidence. Approved preview coverage is `0/24`; independent six-final case coverage is `1/24`. The beta.2 four-mixed-style-board v1 format is superseded and inspection-only. Preview acceptance cannot satisfy the three-complete-workflow target. See the [total task register and schedule](WORK-STATUS.md).
