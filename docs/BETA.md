...
The preview target is 24 separate single-style, six-pose previews of the same white vest. Schema `4.0` separates retained native content from explicit local layout derivatives: `1200×1200` boards, six `360×480` cards, fit-with-padding without stretching, upscaling or subject cropping. Local style labels and AI/non-final disclosures are not native-rendering claims. Historical attempts: 3; machine-layout checked `1/24`, human-approved `0/24`, public `0/24`. The first layout reuses the registered Korean C native image without a new call; its complete visual acceptance remains pending. Old schema 1–3 records keep their original findings and cannot inherit new approval. Previews never satisfy the three-complete-workflow target. See the [total task register and schedule](WORK-STATUS.md).
