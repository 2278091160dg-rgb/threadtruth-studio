...
Preview contact sheets do not count as complete cases. The published beta.3 Release contains 24 previews of the same white vest, ONE style and SIX poses each: machine layout 24/24, maintainer acceptance 24/24, and GitHub publication 24/24. Schema `4.0` produces explicitly disclosed local-layout derivatives from retained native sheets; exact `3:4` applies to display cards, not native geometry. Six corrected selections retain replacement lineage and old rejected drafts remain excluded. Independent six-final coverage remains `1/24` and complete primary cases remain `1/3`.
