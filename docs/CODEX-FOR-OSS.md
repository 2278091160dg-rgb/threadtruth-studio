...
Preview contact sheets do not count as complete cases. The target is 24 previews of the same white vest, ONE style and SIX poses each. Schema `4.0` produces explicitly disclosed local-layout derivatives from retained native sheets; exact `3:4` applies to display cards, not an unverified native geometry claim. Three historical native attempts remain recorded; machine-layout coverage is `1/24`, maintainer visual acceptance is `1/24` and public coverage remains `0/24`; independent six-final coverage remains `1/24`. Old records retain their failures and cannot inherit new approval. The registered Korean C original has been composed without another image-generation call; the maintainer subsequently accepted the displayed visual result. Formal public-use review and publication authorization remain pending.
