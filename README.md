...
- Single-style preview coverage: **1 native attempt, 0/24 qualified, 0/24 human-approved, 0/24 public**. The target is **24 separate contact sheets of the same white vest: ONE style and SIX poses per sheet**. Each native board must be square (`1:1`) including its title, subtitle, six-cell picture area, and footer; each pose cell—not the whole picture area—is portrait `3:4`. The current schema `3.0` collector binds source/rules/pack, exact action-0 prompt, native dimensions, immutable layout/label hashes, observed geometry, and one review per style. It does not generate images or approve reviews. Schema `1.0` and `2.0` runs are historical-only and cannot be mutated or promoted. The prior mixed-style board is separate from the one unqualified single-style attempt. The next gate is one corrected Korean C retry under new explicit authorization; it must pass before the remaining 23 styles. That means 24 future initial calls if none fails, while the consumed attempt stays separate and any failure needs additional retry authority. No software or documentation result increases image counts.
